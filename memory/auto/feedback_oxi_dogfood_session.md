...
- **2026-04-26** — [session — full OXi install/release dogfood, ~6 hours, escotilha/oxibyoxi repo] all three patterns observed and validated within one session. Pierre explicitly named them in his "what's next?" exchanges. (Source: session — escotilha@gmail.com OXi dogfood, 8 commits to main including T0-106, T0-107, T2-46..T2-50 filed; T2-48 implemented + verified end-to-end on live install showing `inserted=42 seeded=1`.)

- **2026-04-26 (continued)** — session extended into b2 PyPI release: filed T0-108 (heavy ML deps) during TestPyPI smoke test, T2-51/T2-52 (release.yml resilience) after re-run failure, then 8 more items (T0-109/110/111, T1-26/27, T2-53/54/55) via Explore-agent research pass. Total: 8 commits during release work + research. b2 published to real PyPI at https://pypi.org/project/oxi-core/0.1.0b2/, tagged `v0.1.0b2`, GitHub Release created. Engine started at concurrency=5 with `--max-cost-per-day 50`; first iteration spawned worktrees for T0-104..T0-108 within seconds. (Source: session — b2 cut via Trusted Publishing OIDC workflow, requested manual approval for `pypi` environment, smoke-tested via fresh-stranger-venv before real publish.)

- **2026-04-26 — pattern reinforcement, late session.** When Pierre wrote "do this now" referring to a list of roadmap items, the natural read was "implement these yourself" but the actual meaning was "I'm specifying the standing policy: always route to engine, never me directly." This is a useful disambiguation rule: in OXi-context, "do this" defaults to "file as roadmap item" not "execute as Claude." Pierre clarified: "Once we engage and we're talking about OXi, it's always about the engine." Updated rule #2 above to make this explicit. The cost of misreading was almost-2-hours of unnecessary direct implementation; the cost of confirming was one round-trip message. Always confirm. (Source: session — direct exchange around 11am after b2 publish completed, before saturate kickoff.)
