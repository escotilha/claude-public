...
- **2026-04-18** — [failure→fix] WhatsApp inbound messages returned "Something went wrong" 12 minutes after migration. Root cause: 877 `/root/.openclaw` absolute paths in `agents/*/sessions/sessions.json` registries — OpenClaw tried to append to `sessionFile: "/root/..."` paths, EACCES mkdir as mary user. Fixed with sed across all session registries + `exec-approvals.json` + `mary-dispatch/index.ts`. Lesson: full-tree grep for old paths is mandatory after a user migration — top-level config + env file is NOT sufficient. (Source: failure — post-migration smoke test via WhatsApp)
