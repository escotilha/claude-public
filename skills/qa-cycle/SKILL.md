...
- `example-project*` → delegate to `/qa-cycle` (ExampleProject has its own dedicated skill)
- `projectb*` → `projectb`
...
3. Known aliases: example-project → `/qa-cycle`, projectb → `/qa-projectb`
...
**If ExampleProject detected:**

```
Skill(skill: "qa-conta", args: "{original_flags}")
```

Done. ExampleProject has its own dedicated skill.
...
Write `$SKILLS_DIR/qa-{project}/SKILL.md` using `/qa-projectb/SKILL.md` as the structural template. Read it first:

```
Read("$SKILLS_DIR/qa-projectb/SKILL.md")
```

The generated skill must include:

1. **YAML frontmatter** — same tools/annotations as qa-projectb, name = `qa-{project}`
...
| Project    | Skill            | Notes                             |
| ---------- | ---------------- | --------------------------------- |
| ExampleProject   | `/qa-cycle`      | Dedicated ExampleProject QA (VPS-based) |
| ProjectB | `/qa-projectb` | Swarm QA with 4 personas          |
...
- **3.0.0**: Autonomous opus orchestrator
  ...
  - Detects ExampleProject → delegates to `/qa-cycle` (not `/qa-cyclebly`)
  - No ExampleProject-specific code in this skill
  - Full cycle loop until 100% coverage or 10 cycles
  - Generates self-contained `/qa-{project}` skills using qa-projectb as template
  - Memory trail for instant routing on subsequent runs
- **2.0.0**: Project-agnostic master (sonnet orchestrator)
- **1.0.0**: ExampleProject-specific QA cycle
