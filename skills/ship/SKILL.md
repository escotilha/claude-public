...
7. Detect QA skill: if project has qa-projectb → use it, if qa-cycle → use it, else → generic Chrome DevTools testing
...
```json
{
  "project": {
    ...
    "qaSkill": "/qa-projectb|/qa-cycle|/fulltest-skill|generic",
...
> - **Code Orchestration (sandbox over discrete tool calls):** _"For services with large API surfaces (Supabase, GitHub API, BankIntegration, cloud consoles, anything with >20 endpoints), prefer writing a single Bash/Python script that composes multiple API calls and returns only the distilled result, instead of calling 10+ individual MCP tools and ingesting raw output. ~37% token reduction on complex workflows."_
...
5. **Project-specific QA:** Based on detected `project.qaSkill`:
   - If `/qa-projectb` available AND project is ProjectB → invoke it
...
| QA skill        | Phase 5             | Auto-detected: /qa-projectb, /qa-cycle, /fulltest-skill, or generic Chrome DevTools |
