...
Creates professional PowerPoint presentations for M&A board approval in Example brand style.
...
Analyzes M&A opportunities and scores them 0-10 against Example's investment criteria.
...
Generates comprehensive Example portfolio financial reports and presentations from individual company Excel files.
...
These skills integrate with the `example-mna` MCP server:

**Triage Analyzer** → `mcp__example-mna__triage_deal`
**Proposal Generator** → `mcp__example-mna__generate_proposal`
**Committee Presenter** → `mcp__example-mna__create_presentation`

See `../mcp-servers/example-mna/README.md` for MCP server documentation.
