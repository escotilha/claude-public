...
4. **List existing design docs for this project:**
   ...
   ls -t ~/.claude-setup/designs/$SLUG/*-design-*.md 2>/dev/null
   ```
...
```bash
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.claude-setup/designs/$SLUG/*-design-*.md 2>/dev/null
```
...
Before writing the design doc, synthesize the signals you observed during the session. These will appear in the design doc ("What I noticed").
...
```bash
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")
mkdir -p ~/.claude-setup/designs/$SLUG
USER=$(whoami)
...
```bash
PRIOR=$(ls -t ~/.claude-setup/designs/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```

If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it.

Write to `~/.claude-setup/designs/{slug}/{user}-{branch}-design-{datetime}.md`:
...
Use the Agent tool to dispatch an independent reviewer (model: sonnet). The reviewer has fresh context and cannot see the brainstorming conversation — only the document.
...
**Convergence guard:** If the reviewer returns the same issues on consecutive iterations, stop the loop and persist those issues as "Reviewer Concerns" in the document.

If the subagent fails or is unavailable — skip the review loop entirely. The document is already written; the review is a quality bonus, not a gate.
...
One paragraph that references specific things the user said during the session — quote their words back to them.
...
Suggest the next step based on the design doc:

- **`/deep-plan`** for complex implementation planning — research → plan → implement
- **`/ship`** for end-to-end feature shipping — spec → implement → test → ship
- **`/cto`** for architecture review before building
- **`/first-principles`** if the problem needs further decomposition

The design doc at `~/.claude-setup/designs/` is discoverable by downstream skills.
