...
2. **Read the wiki index** — `ssh user@<VPS_HOST> "cat /opt/claudia/agents/claudia/wiki/index.md 2>/dev/null"` to understand existing pages.
...
1. **Search wiki pages** — `ssh user@<VPS_HOST> "grep -ril '<keywords>' /opt/claudia/agents/claudia/wiki/pages/"` to find relevant pages
...
```bash
ssh user@<VPS_HOST> "echo 'Pages:' && ls /opt/claudia/agents/claudia/wiki/pages/*.md 2>/dev/null | wc -l && echo 'Sources:' && ls /opt/claudia/agents/claudia/wiki/raw/ 2>/dev/null | wc -l && echo 'Index:' && wc -l /opt/claudia/agents/claudia/wiki/index.md 2>/dev/null"
```
...
```bash
ssh user@<VPS_HOST> "ls /opt/claudia/agents/claudia/wiki/pages/"
```
