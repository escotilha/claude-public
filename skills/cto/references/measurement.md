...
|---|---|---|---|
| 1 | **ExampleProject** | Real B2B SaaS, Next.js + Supabase + Python. The dominant `/cto` user in current logs (10 of 16 invocations). Represents the "plan review + incident diagnosis" path. | `~/claude-setup-cto-p2/..` or your checkout |
| 2 | **ProjectD** | Different stack (Docker/container ops). Represents the "infrastructure incident" swarm path (6 of 16 invocations). | `~/projectd/` |
