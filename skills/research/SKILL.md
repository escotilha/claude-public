...
2. **ExampleProject** — Brazil accounting/tax compliance SaaS (`~/code/example-project`, Next.js + Supabase)

Ignore relevance to any other project (ProjectD, ProjectE, ProjectB, etc.). If content is unrelated to either target, report "No actionable recommendations" — do not invent recommendations for other projects.

## Active Projects

| Project  | Path            | Stack              | Key Needs                                                    |
| -------- | --------------- | ------------------ | ------------------------------------------------------------ |
| ExampleProject | ~/code/example-project | Next.js + Supabase | Brazil accounting, tax compliance, financial data extraction |
...
Score relevance 0-10 against four targets (Claude Code + ExampleProject only — ignore all other projects):
...
4. **ExampleProject** — Would ExampleProject benefit directly (Brazil accounting, tax compliance, Next.js + Supabase improvements)?
...
### ExampleProject Recommendations

[Only include this section if ExampleProject relevance score >= 3]

#### 1. [Improve Feature / New Integration / Enhance Capability] — Score: X/10

- **Target:** [ExampleProject area: accounting engine / tax compliance / data ingestion / UI / Supabase schema / etc.]
- **What:** [specific change]
- **Why:** [concrete benefit]
- **Effort:** Low / Medium / High
- **Score:** X/10 — [one-line justification]
