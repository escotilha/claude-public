---
title: "feat: Merge ProjectE variant into main repo"
type: feat
date: 2026-02-14
---

# feat: Merge ProjectE Variant into Main Repo

## Overview

Consolidate the `last30days-projecte` project into `last30days-skill-private` so there's one unified Python engine powering both the main skill (Claude Code / Codex) and an "open" variant with watchlist, briefing, history, and built-in web search. The open variant also gets YouTube and Bird CLI — features the main project already has but projecte was built before they existed.
...
- **`last30days-projecte`** (Feb 10) — SQLite store, watchlist, briefings, 3 web search backends (Parallel AI, Brave, OpenRouter). But frozen without YouTube or latest engine improvements.
...
Use `last30days-skill-private` as the base (it's 4 days newer with better code) and port the ProjectE-exclusive features in:

### What gets ported from ProjectE
...
- **`watchlist.py`**: Remove ProjectE cron-specific code, make cron setup generic (launchd on macOS, systemd on Linux, or manual cron)
- **`briefing.py`**: No changes needed
- **`scripts/lib/env.py`**: Merge ProjectE's web search key support (`PARALLEL_API_KEY`, `BRAVE_API_KEY`, `OPENROUTER_API_KEY`) and `has_web_search_keys()` / `get_web_search_source()` functions into main's env.py. Drop the ProjectE config loader (`~/.projecte/projecte.json`) — just use env vars and `~/.config/last30days/.env`
- **`scripts/last30days.py`**: Add ProjectE's `_search_web()` function so the script can do web search natively when API keys are available (instead of always delegating to the assistant)

### What gets DROPPED from ProjectE

| File | Why |
|------|-----|
| `scripts/cron_setup.py` | Too ProjectE-platform-specific. Replace with generic scheduling docs. |
| ProjectE config loader in `env.py` | `~/.projecte/projecte.json` path is platform-specific. Use env vars instead. |
| `.clawhubignore` | ProjectE marketplace artifact, not needed in unified repo |
...
│   ├── store.py                      # SQLite accumulator (from projecte)
│   ├── watchlist.py                  # Watchlist CLI (from projecte, genericized)
│   ├── briefing.py                   # Briefing generator (from projecte)
│   └── lib/
│       ├── ... (existing files)
│       ├── brave_search.py           # NEW from projecte
│       ├── parallel_search.py        # NEW from projecte
│       ├── openrouter_search.py      # NEW from projecte
...
Main's `env.py` is the base. Add from ProjectE:
...
Main's `last30days.py` is the base. Add from ProjectE:
...
- [x] `scripts/watchlist.py` ported with generic scheduling (no ProjectE cron dependency)
...
|------|-----------|------------|
| ProjectE's store.py has import dependencies we don't have | Low | store.py uses only stdlib (sqlite3, json, datetime). Self-contained. |
...
| watchlist.py depends on ProjectE cron API | High | Known — strip cron_setup.py dependency, replace with generic docs for launchd/systemd/crontab. |
...
- `variants/open/references/research.md` — One-shot research instructions (from projecte, updated with YouTube)
- `variants/open/references/watchlist.md` — Watchlist management instructions (from projecte)
- `variants/open/references/briefing.md` — Briefing mode instructions (from projecte)
- `variants/open/references/history.md` — History query instructions (from projecte)
- `variants/open/context.md` — Agent memory template
- `scripts/store.py` — SQLite accumulator (from projecte, as-is)
- `scripts/watchlist.py` — Watchlist CLI (from projecte, genericized)
- `scripts/briefing.py` — Briefing generator (from projecte, as-is)
- `scripts/lib/brave_search.py` — Brave Search API (from projecte)
- `scripts/lib/parallel_search.py` — Parallel AI search (from projecte)
- `scripts/lib/openrouter_search.py` — OpenRouter/Sonar Pro search (from projecte)
...
### Internal
- ProjectE plan: `/Users/mvanhorn/last30days-projecte/docs/plans/2026-02-10-feat-projecte-last30days-skill-plan.md` (989 lines, comprehensive spec)
- Codex compat plan: `docs/plans/2026-02-14-feat-codex-skill-compatibility-plan.md` (portable paths, platform-neutral text)
- ProjectE source: `/Users/mvanhorn/last30days-projecte/`

### Key files to port
- `store.py`: `/Users/mvanhorn/last30days-projecte/scripts/store.py` (20KB, SQLite with FTS5)
- `watchlist.py`: `/Users/mvanhorn/last30days-projecte/scripts/watchlist.py` (10KB)
- `briefing.py`: `/Users/mvanhorn/last30days-projecte/scripts/briefing.py` (8KB)
- `brave_search.py`: `/Users/mvanhorn/last30days-projecte/scripts/lib/brave_search.py` (6KB)
- `parallel_search.py`: `/Users/mvanhorn/last30days-projecte/scripts/lib/parallel_search.py` (4KB)
- `openrouter_search.py`: `/Users/mvanhorn/last30days-projecte/scripts/lib/openrouter_search.py` (7KB)
- `env.py` (projecte version): `/Users/mvanhorn/last30days-projecte/scripts/lib/env.py` (9KB — has web search key functions)
