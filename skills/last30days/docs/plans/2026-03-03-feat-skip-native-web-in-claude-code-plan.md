...
This is redundant. Claude's WebSearch is better and free. In ProjectE, there's no WebSearch tool, so native backends are essential there.
...
### 3. ProjectE / `--agent` mode
- [ ] No changes needed — ProjectE invocations don't read SKILL.md, they call the script directly without `--no-native-web`, so Parallel AI/Brave/OpenRouter still work

## Acceptance Criteria

- [ ] Claude Code sessions: script skips Parallel AI, Claude uses WebSearch (no API credits spent)
- [ ] ProjectE sessions: script still uses Parallel AI / Brave / OpenRouter as before
