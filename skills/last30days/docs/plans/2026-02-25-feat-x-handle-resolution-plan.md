...
- **20% ProjectE** - agent has web search capability
...
Move handle resolution to the **SKILL.md agent layer**. The agent (Claude or ProjectE) already has `WebSearch` as an allowed tool. It does a single WebSearch before running the Python script, extracts the handle, and passes it as a CLI argument.
...
1. **100% availability** - Claude/ProjectE always has WebSearch
...
### ProjectE Variant Changes

Same instruction added to `variants/open/references/research.md` (or inline in the open SKILL.md routing). ProjectE agents also have WebSearch available.
...
- [x] ProjectE variant updated with same instructions
...
| `variants/open/references/research.md` | Same handle resolution instructions for ProjectE |
...
- ProjectE variant: `variants/open/SKILL.md`
