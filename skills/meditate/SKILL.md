---
name: meditate
description: "End-of-session judgment extraction — learnings, patterns, mistakes. Triggers: meditate, reflect on session, extract learnings."
user-invocable: true
context: fork
model: sonnet
effort: medium
...
## Phase 0b: Transcript Synthesis (GBrain "dream" Pattern)

> Adapted from GBrain v0.23.0 (2026-04-30). When `/meditate` runs as a scheduled overnight job — or when invoked with `synthesize=<path>` — it can ingest **raw transcripts** (Claude Code session logs, OpenClaw runs, meeting notes, ad-hoc files) and convert them into tier-routed memory pages without a live session to reflect on. This is the mechanism that produces 10-15 reflections/month autonomously.

### When this phase runs

Invoke this phase **instead of** Phase 1 when any of these are true:

- `synthesize=<path-or-glob>` is in the invocation arguments (explicit input)
- The skill was triggered by `/schedule` overnight with no live session context
- A `~/.claude-setup/memory/auto/_inbox/` directory exists with files dropped in by user or hooks

If none apply, skip this phase and go straight to Phase 1.

### Sources to ingest

| Source | Path/glob | Tier |
| --- | --- | --- |
| Claude Code session transcripts | `~/.claude/transcripts/*.jsonl` modified in last 24h | episodic + semantic |
| OpenClaw run logs | `~/code/oxi/runs/*/transcript.md` | episodic + semantic |
| Compaction summaries | `~/.claude/compactions/*.md` last 24h | semantic |
| User drop folder | `~/.claude-setup/memory/auto/_inbox/*` | depends on content |

### Process

1. **Cap intake at 5 sources per run.** If more are present, sort by mtime descending and take the freshest. Older sources stay for the next nightly run.
2. **Per source, run a single Sonnet pass** (not Haiku — synthesis needs nuance). For each source, extract:
   - **Reflections** (1-3 per source) — short observations about what happened. Map to `auto/episodic/`.
   - **Originals/ideas** (0-2 per source) — novel design or product ideas the user surfaced. Map to `auto/working/` with a 14-day decay note.
   - **Patterns** (0-1 per source) — reusable techniques. Hold these as candidates; they only get promoted to `auto/semantic/` if Phase 0c (motif check) finds 3+ corroborating sources.
3. **Apply the same scoring threshold as nudge mode** (>= 7) — synthesized findings without a live conversation are noisier, so use the higher bar.
4. **Move ingested files** to `~/.claude-setup/memory/auto/_inbox/_processed/{date}/` so they don't get re-ingested on the next run.

### Phase 0c: Motif Check (cross-source pattern detection)

After synthesis but before writing pattern entities to `auto/semantic/`:

```
For each pattern candidate:
  count = number of distinct sources in this run + last 7 days of episodic memories that mention this motif
  IF count >= 3:
    → Write to auto/semantic/pattern_{slug}.md
    → Cite all corroborating sources in the timeline
  ELSE:
    → Hold as a working/ note; do not promote
```

This is the same `≥3 motifs → pattern page` heuristic that lives in `/memory-consolidation` Step 3.5 — meditate enforces it at ingest time so the consolidator doesn't have to retroactively promote.

### Cost envelope

Target: ~$1-2 per overnight run. Sonnet 4.6 for synthesis (5 sources × ~3K tokens in / 2K out = ~$0.60). Haiku 4.5 for the motif/score verdict (~$0.10). Stay under $2 even with full inbox.

### Cooldown

Run at most **once per 12 hours**. Track via mtime on `~/.claude-setup/memory/auto/_inbox/_processed/.last_run`. Synthesizing the same transcripts twice in a day produces near-duplicate reflections.

### Nudge mode interaction

Skip Phase 0b entirely in nudge mode — it's a heavyweight overnight phase, not a periodic check.
